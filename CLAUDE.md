# Bare Minimum Theory: Claude Agent Context
...
├── lib/
│   ├── components/     # UI components (ChordBuilder, Progression, Palette, etc.)
...
State is managed via Svelte 5 runes in `src/lib/stores/`:
...
- `addChord(chord)`, `updateChord(index, chord)`, `removeChord(index)`
- `addToPalette(chord)`, `removeFromPalette(index)`
- `setScale(key, mode)`, `clearScale()`
- `loadProgressionToCanvas(progression)`
...
- `setMIDIEnabled(bool)`, `setMIDIDevice(id)`
- `setClockSyncEnabled(bool)`, `setClockInputDevice(id)`
...
- `setRandomizeOption(key, bool)`
- `setPianoVisible(bool)`, `setActiveNotes(notes)`

## Audio & MIDI Implementation

- **Audio**: Uses `Tone.PolySynth` in `audio-playback.ts`. Requires `await Tone.start()` on user gesture.
- **MIDI Output**: Web MIDI API. Uses `midi-output.ts` to send notes to virtual ports.
- **MIDI Clock**: `midi-clock.ts` handles DAW transport/tempo synchronization.
- **MIDI Export**: `midi-writer-js` generates `.mid` files in `midi-export.ts`.
- **Persistence**:
...
- **Command**: `bun run test` runs the full Vitest suite.
- **Note**: Avoid `bun test`; always use `bun run test` to ensure Svelte processing.
